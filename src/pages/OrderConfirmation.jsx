  ...
  return (
    <div className="min-h-screen canvas-bg text-navy flex items-center justify-center px-6 py-16">
      <div className="relative z-10 max-w-lg w-full text-center">
        ...
        <p className="text-lg text-navy/50 mb-8">Thank you for your order. We'll be in touch shortly with payment details and shipping updates.</p>
        {order && (
          <div className="card-painted-round rounded-[2rem] p-6 text-left mb-8">
            <h2 className="font-black mb-4">Order Details</h2>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-navy/40">Order #</span><span className="font-bold">{order.order_number}</span></div>
              <div className="flex justify-between"><span className="text-navy/40">Email</span><span className="font-bold">{order.email}</span></div>
              <div className="flex justify-between"><span className="text-navy/40">Ship to</span><span className="font-bold">{order.shipping_city}, {order.shipping_state} {order.shipping_zip}</span></div>
