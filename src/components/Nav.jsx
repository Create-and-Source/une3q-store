...
export default function Nav() {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 bg-canvas/95 backdrop-blur-lg">
        ...
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
            ...
            {[
              ...
              >
                ...
                {location.pathname === link.to && (
                  <span className="absolute -bottom-1 left-0 right-0 h-[3px] bg-lime rounded-full" />
                )}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-4">
            <Link to="/cart" className="relative hidden md:flex items-center gap-2 bg-purple text-white px-5 py-3 rounded-full font-bold shadow-lg color-shadow hover:scale-105 transition-all">
              <ShoppingBag size={18} />
              Cart
              {itemCount > 0 && (
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="md:hidden fixed inset-0 top-[69px] z-40 bg-canvas/98 backdrop-blur-xl">
          <div className="px-6 py-8 flex flex-col gap-5">
            {[
              ...
              <Link
                ...
                className="text-2xl font-black text-navy hover:text-purple transition border-b-2 border-cream-dark pb-4"
              >
                {link.label}
              </Link>
            ))}
